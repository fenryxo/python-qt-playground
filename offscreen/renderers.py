...
from PySide2.QtCore import QObject, Signal, QSize, QUrl, QTimer, QCoreApplication, Slot, QEvent
...
class QmlOffscreenRenderer(QObject):
    ...
    _framebuffer: Optional[QOpenGLFramebufferObject] = None
    _component: QQmlComponent = None
    _rootItem: QQuickItem = None

    def __init__(self, qmlUrl: QUrl):
        super().__init__()
        self.qmlUrl = qmlUrl
    ...
    def resize(self, size: QSize) -> None:
        ...
        if self._rootItem and self.context.makeCurrent(self._surface):
    ...
    def _attachRootItem(self):
        ...
        self._window.contentItem().forceActiveFocus()
        self._updateSizes()
        self.context.makeCurrent(self._surface)
